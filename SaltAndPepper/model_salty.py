...
class DiffusionNetStepSalt(nn.Module):
    def __init__(self, rbf, n_channels=1, n_filters=5, filter_size=5):
        super(DiffusionNetStepSalt, self).__init__()
...
class DiffusionNetSalt(nn.Module):
    def __init__(self, n_rbf=63, T=5, **dnet_args):
        super(DiffusionNetSalt, self).__init__()
        ...
        self.dnets = nn.ModuleList([DiffusionNetStepSalt(rbfs, **dnet_args) for i in range(T)])
